...
import { CloseSmallIcon, InfoCircleOutlineIcon } from "@sapo-presentation/sapo-ui-components";
  ...
  return (
    <Fragment>
      <MuiAutocomplete
        ...
        onBlur={() => setText("")}
        classes={{
          root: withoutMinHeight ? classes.rootWithoutMinHeight : classes.root,
          tag: classes.tag,
          paper: classes.paper,
        }}
        renderInput={(params) => {
          ...
          return value.map((option, index) => {
            ...
            return (
              <ChipComponent
                ...
                style={{ fontSize: 14 }}
                deleteIcon={disabled ? <Fragment /> : <CloseSmallIcon />}
                {...tagsProps}
                disabled={false}
                onDelete={(e) => {
                  const newVal = [...value];
